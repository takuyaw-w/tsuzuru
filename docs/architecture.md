...
`examples:check` remains the standalone example gate and builds workspace dependencies through each example package. `examples:check:self` is the release-readiness example gate; it assumes `packages:build` has already run and uses example `typecheck:self` / `build:self` to avoid rebuilding packages.
