...
この example は、`@tsuzuru/plugin-std-audio` の `@startBgm("assetId")` / `@stopBgm()` / `@se("assetId")` / `@voice("assetId")` が Preact app からどう見えるかを確認するための最小構成です。
...
任意の音声ファイルを置く場合は、`src/` 配下ではなく `public/` 配下の次の path を使えます。

```txt
examples/preact-std-audio/public/assets/audio/bgm/main_theme.mp3
examples/preact-std-audio/public/assets/audio/se/click.mp3
examples/preact-std-audio/public/assets/audio/se/confirm.mp3
examples/preact-std-audio/public/assets/audio/voice/alice_001.mp3
examples/preact-std-audio/public/assets/audio/voice/alice_002.mp3
```
