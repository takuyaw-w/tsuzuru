...
function RuntimeApp({ document }: RuntimeAppProps) {
  ...
  const plugins = useMemo(() => [createStdAudioPlugin()], []);
  const commandHandlers = useMemo(() => createStdAudioCommandHandlers(), []);
  ...
  return (
    <main className="app">
      <h1>Tsuzuru Preact std-audio</h1>
...
function compileScenario(source: string): DocumentResult {
  const parsed = parseTzr(source, { filePath: "examples/preact-std-audio/scenario/main.tzr" });
  if (!parsed.ok) {
    return { ok: false, message: formatDiagnostics(parsed.errors) };
  }

  const compiled = compileTzr(parsed.document, { pluginCommands: stdAudioPluginCommands });
  if (!compiled.ok) {
