...
describe("createProject", () => {
  ...
  it("generates a creator-facing app shell", async () => {
    ...
    const gameSource = await readFile(join(root, "my-game", "src", "game.ts"), "utf8");
    ...
    expect(appSource).toContain('import { game } from "./game.js"');
    expect(appSource).toContain("<TitleScreen");
    expect(appSource).toContain("<GameRoot game={game} />");
    expect(appSource).not.toContain("./assets");
    expect(appSource).not.toContain("../scenario/main.tzr");
    ...
    expect(gameSource).toContain('import { createStandardGameStorage } from "@tsuzuru/standard-game-storage"');
    expect(gameSource).toContain('import scenario from "../scenario/main.tzr"');
    expect(gameSource).toContain('import { assets } from "./assets.js"');
    expect(gameSource).toContain("project: projectIdentity");
    expect(gameSource).toContain("storage: createStandardGameStorage");
    expect(gameSource).toContain("slots: 3");
    expect(gameSource).not.toContain("{{projectName}}");
    expect(gameSource).not.toContain("storagePrefix");
    expect(gameSource).not.toContain("@tsuzuru/standard-game-storage/src");
    expect(gameSource).not.toContain("localStorage");
    expect(gameSource).not.toContain("RuntimeSave");
    expect(gameSource).not.toContain("retainedMessageEvent");
    expect(gameSource).not.toContain("migration");
    expect(gameSource).not.toContain("readEntry");
    expect(gameRootSource).toContain("TsuzuruGame");
    expect(gameRootSource).toContain("scenario={game.scenario}");
    expect(gameRootSource).toContain("assets={game.assets}");
    ...
    expect(readmeSource).toContain("src/game.ts");
